...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (successCounter != 10 && unsuccesCounter != 100) {
            ...
            WinnerTakesAllNeuron[] kohonens = new WinnerTakesAllNeuron[numberOfNeurons];
            for (int i = 0; i < numberOfNeurons; i++) {
                kohonens[i] = new WinnerTakesAllNeuron(numberOfInputs);
            }
            ...
            if (ages != learnLimit) {
                ...
                for (int i = 0; i < numberOfFlowers; i++) {
                    winner = getWinner(kohonens, LearningDataSet.flowerLearn[i][0]);
                ...
                for (int i = 0; i < numberOfFlowers; i++) {
                    for (int j = 0; j < numberOfTestSamples; j++) {
                        winner = getWinner(kohonens, LearningDataSet.flowerTest[i][j]);
    ...
    //uczenie sieci
    private static int learn(WinnerTakesAllNeuron[] kohonens) {
        ...
        while (!isUnique(winners)) {	//dopóki sieć się nauczy
            ...
            for (int i = 0; i < numberOfFlowers; i++) {
                for (int j = 0; j < numberOfLearnSamples; j++) {
                    winner = getWinner(kohonens, LearningDataSet.flowerLearn[i][j]);
                    kohonens[winner].learn(LearningDataSet.flowerLearn[i][j], learningRate);
                }
            }
            ...
            for (int i = 0; i < numberOfFlowers; i++) {
                for (int j = 0; j < numberOfLearnSamples; j++) {
                    winners[i][j] = getWinner(kohonens, LearningDataSet.flowerLearn[i][j]);
                }
            }
    ...
    private static int getWinner(WinnerTakesAllNeuron[] kohonens, double[] vector) {
